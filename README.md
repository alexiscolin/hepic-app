...
Hepic App is an SAP generated by an PWA system. As a OAuth application seved by thirds parties, this SPA must deal with 3Ps and OS's constraints. Nowadays, iOS 11.4 has some issues to deals with 3P OAuth because of page redirection [Report on GitHub](https://github.com/nolanlawson/pinafore/issues/45).

### FIX - WORKAROUND

in order to fix that issue, we make the choice to avoid using standalone app in iOS device until this bug will be fixed by Apple. The manifest is removed on Apple Device.
